...
// Fungsi IP Simulasi (Sebagai Device ID)
function getUserIP() {
...
function handleRoute() {
    ...
    // Sembunyikan semua tab & nonaktifkan navigasi bawah
    document.querySelectorAll('.tab-view').forEach(t => t.classList.remove('active'));
    document.querySelectorAll('.nav-item, .nav-fab').forEach(i => i.classList.remove('active'));
    
    // Jika belum login, hanya boleh akses landing page atau /sendv2
    ...
    // Aktifkan icon di navigasi bawah
    ...
    // Trigger update UI spesifik sesuai halaman
    if (path === '/inbox') {
        renderInbox();
        markInboxRead();
    } else if (path === '/public') {
...
// Inisialisasi Kerangka HTML Utama
function renderAppShell() {
    ...
    // Tarik data awal dari Firebase jika user ada
...
function attachEvents() {
    ...
    // Fungsi Global (dipanggil dari string HTML render)
